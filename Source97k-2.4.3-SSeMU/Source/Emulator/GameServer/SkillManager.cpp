...
int CSkillManager::GetSkillFrustrum(int* SkillFrustrumX, int* SkillFrustrumY,BYTE angle,int x,int y,float sx,float sy) // OK
{	
	vec3_t p[4];
	
	Vector(-sx, sy, 0.f, p[0]);
	Vector(sx, sy, 0.f, p[1]);
...
bool CSkillManager::CheckSkillTarget(LPOBJ lpObj,int aIndex,int bIndex,int type) // OK
{
	...
	if(gGuild.gObjTargetGuildWarCheck(lpObj,&gObj[SummonIndex]) != 0)
	{
		return 1;
	}
...
int CSkillManager::AddSkillWeapon(LPOBJ lpObj,int index,int level) // OK
{
	...
	if((slot=this->AddSkill(lpObj,(skill=index))) >= 0)
	{
		this->GCSkillAddSend(lpObj->Index,slot,skill,(BYTE)level);return 1;
	}
	else
	{
		return 0;
...
int CSkillManager::DelSkillWeapon(LPOBJ lpObj,int index,int level) // OK
{
	...
	{
		this->GCSkillDelSend(lpObj->Index,slot,skill,(BYTE)level);return 1;
	}
	else
	{
		return 0;
...
int CSkillManager::AddSkill(LPOBJ lpObj,int index) // OK
{
	...
	for(int n=0;n < MAX_SKILL_LIST;n++)
	{
		...
		{
			return ((lpObj->Skill[n].Set(index)==0)?-1:n);
		}
	}
...
void CSkillManager::UseAttackSkill(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	{
		if(this->CheckSkillRequireWeapon(lpObj,lpSkill->m_index) == 0)
		{
			return;
		}

		if(lpSkill->m_index == SKILL_TWISTING_SLASH || lpSkill->m_index == SKILL_RAGEFUL_BLOW || lpSkill->m_index == SKILL_DEATH_STAB)
		{
	...
	if(lpObj->SkillNovaState != 0 && lpSkill->m_index != SKILL_NOVA && this->GetSkill(lpObj,SKILL_NOVA) != 0)
	{
		this->RunningSkill(aIndex,0,this->GetSkill(lpObj,SKILL_NOVA),(BYTE)lpObj->X,(BYTE)lpObj->Y,0,0);
		return;
	}

	if(lpSkill->m_index == SKILL_NOVA && bIndex != SKILL_NOVA_START)
	{
...
void CSkillManager::UseDurationSkillAttack(int aIndex,int bIndex,CSkill* lpSkill,BYTE x,BYTE y,BYTE dir,BYTE angle) // OK
{
	...
	if(lpObj->Type == OBJECT_USER && this->CheckSkillRequireWeapon(lpObj,lpSkill->m_index) == 0)
	{
		return;
	}
...
bool CSkillManager::SkillManaShield(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	gEffectManager.AddEffect(lpTarget,0,this->GetSkillEffect(lpSkill->m_index),count,value1,0,0,0);
...
bool CSkillManager::SkillGreaterLife(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if(OBJECT_RANGE(lpObj->PartyNumber) == 0)
	{
		...
		gEffectManager.AddEffect(lpObj,0,this->GetSkillEffect(lpSkill->m_index),count,value1,0,0,0);
	...
	else
	{
		...
		for(int n=0;n < MAX_PARTY_USER;n++)
		{
			...
			gEffectManager.AddEffect(&gObj[index],0,this->GetSkillEffect(lpSkill->m_index),count,value1,0,0,0);
...
void CSkillManager::CGMultiSkillAttackRecv(PMSG_MULTI_SKILL_ATTACK_RECV* lpMsg,int aIndex,bool type) // OK
{
	...
	if(lpSkill->m_index != SKILL_FLAME && lpSkill->m_index != SKILL_TWISTER && lpSkill->m_index != SKILL_EVIL_SPIRIT && lpSkill->m_index != SKILL_HELL_FIRE && lpSkill->m_index != SKILL_AQUA_BEAM && lpSkill->m_index != SKILL_BLAST && lpSkill->m_index != SKILL_INFERNO && lpSkill->m_index != SKILL_TRIPLE_SHOT
	&& lpSkill->m_index != SKILL_IMPALE && lpSkill->m_index != SKILL_MONSTER_AREA_ATTACK && lpSkill->m_index != SKILL_PENETRATION && lpSkill->m_index != SKILL_FIRE_SLASH)
	{
		return;
	}
	...
	if(lpObj->Type == OBJECT_USER && lpSkill->m_index == SKILL_TRIPLE_SHOT)
	{
	...
	for(int n=0;n < lpMsg->count;n++)
	{
		...
		else if(lpSkill->m_index == SKILL_FIRE_SLASH)
		{
			gAttack.Attack(lpObj,lpTarget,lpSkill,1,1,0,0,0);
		}
		else
...
void CSkillManager::GCSkillAddSend(int aIndex,BYTE slot,int skill,BYTE level) // OK
{
#if(GAMESERVER_UPDATE<99)
	LPOBJ lpObj = &gObj[aIndex];
	this->GCSkillListSend(lpObj);
#else
	BYTE send[256];
	...
	info.skill[0] = (BYTE)(skill & 0xFF);

	info.skill[1] = (BYTE)((level << 3) | ((skill >> 8) & 7));
...
void CSkillManager::GCSkillDelSend(int aIndex,BYTE slot,int skill,BYTE level) // OK
{
#if(GAMESERVER_UPDATE<99)
	LPOBJ lpObj = &gObj[aIndex];
	this->GCSkillListSend(lpObj);
...
void CSkillManager::GCSkillListSend(LPOBJ lpObj) // OK
{
	BYTE send[1024];
	...
	for(int n=0;n < MAX_SKILL_LIST;n++)
	{
		...
		info.skill[0] = (BYTE)(lpObj->Skill[n].m_index & 0xFF);

		info.skill[1] = (BYTE)((lpObj->Skill[n].m_index >> 8) & 7);
