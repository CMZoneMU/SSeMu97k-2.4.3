// Source 97k Kayito Update 21 -> 97k SSeMU - Menu de Opções Avançadas
#include "stdafx.h"
...
// DUMMY STRUCTS PARA EVITAR ERROS DE COMPILACAO EM 97.04 SEM QUEBRAR SINTAXE

struct DummyPair { int first; int second; };
struct DummyWindow {
    bool m_WindowMode = false;
    bool m_Borderless = false;
    DummyPair iResolutionValues[20];
    DummyWindow() {
        iResolutionValues[0] = {640, 480};
        iResolutionValues[1] = {800, 600};
        iResolutionValues[2] = {1024, 768};
        iResolutionValues[3] = {1280, 1024};
        iResolutionValues[4] = {1366, 768};
        iResolutionValues[5] = {1440, 900};
        iResolutionValues[6] = {1600, 900};
        iResolutionValues[7] = {1680, 1050};
        iResolutionValues[8] = {1920, 1080};
    }
    void ChangeWindowState(bool a, bool b, int c) {
        if (c >= 0 && c <= 8) {
            *(int*)0x055C9E38 = c; // m_Resolution

            WindowWidth = iResolutionValues[c].first;
            WindowHeight = iResolutionValues[c].second;
            ...
            LONG PosX = ((GetSystemMetrics(SM_CXSCREEN)) / 2) - ((rc.right - rc.left) / 2);
            LONG PosY = ((GetSystemMetrics(SM_CYSCREEN)) / 2) - ((rc.bottom - rc.top) / 2);

            SetWindowPos(g_hWnd, NULL, PosX, PosY, rc.right - rc.left, rc.bottom - rc.top, SWP_NOZORDER | SWP_SHOWWINDOW);

            HKEY hKey;
            if (RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\Webzen\\Mu\\Config", 0, KEY_SET_VALUE, &hKey) == ERROR_SUCCESS) {
                DWORD res = c;
                RegSetValueEx(hKey, "Resolution", 0, REG_DWORD, (const BYTE*)&res, sizeof(res));
                RegCloseKey(hKey);
            }
        }
    }
...
COptionsMenu::COptionsMenu()
{
	...
	m_bWhisperSound = (GetPrivateProfileInt("Sound", "WhisperSound", 1, ".\\Config.ini") != 0);
...
COptionsMenu::~COptionsMenu()
{
	...
	wsprintf(Text, "%d", (m_bWhisperSound) ? 1 : 0);
	WritePrivateProfileString("Sound", "WhisperSound", Text, ".\\Config.ini");
...
const char* COptionsMenu::GetOptionText(int index)
{
	int lang = gLanguage.LangNum; // 0 = Eng, 1 = Spn, 2 = Por
	switch (index)
	{
		case 385: return (lang == 2) ? "Opcoes" : ((lang == 1) ? "Opciones" : "OPTIONS");
		case 386: return (lang == 2) ? "Ataque Automatico" : ((lang == 1) ? "Ataque Automatico" : "Auto Attack");
		case 387: return (lang == 2) ? "Som no Sussurro" : ((lang == 1) ? "Sonido de Susurro" : "Whisper Sound");
		case 388: return (lang == 2) ? "Fechar" : ((lang == 1) ? "Cerrar" : "Close");
		case 919: return (lang == 2) ? "Geral" : ((lang == 1) ? "General" : "General");
		case 920: return (lang == 2) ? "Tela" : ((lang == 1) ? "Pantalla" : "Screen");
		case 921: return (lang == 2) ? "Fonte" : ((lang == 1) ? "Fuente" : "Font");
		case 922: return (lang == 2) ? "Som" : ((lang == 1) ? "Sonido" : "Sound");
		case 923: return (lang == 2) ? "Musica" : ((lang == 1) ? "Musica" : "Music");
		case 924: return (lang == 2) ? "Restaurar Fonte" : ((lang == 1) ? "Restablecer Fuente" : "Reset Font");
		case 925: return (lang == 2) ? "Voltar" : ((lang == 1) ? "Volver" : "Back");
		case 926: return (lang == 2) ? "Anti-Lag" : ((lang == 1) ? "Antilag" : "Antilag");
		case 927: return (lang == 2) ? "Sombras" : ((lang == 1) ? "Sombras" : "Shadows");
		case 928: return (lang == 2) ? "Objetos" : ((lang == 1) ? "Objetos" : "Objects");
		case 929: return (lang == 2) ? "Chao" : ((lang == 1) ? "Suelo" : "Floor");
		case 930: return (lang == 2) ? "Habilidades" : ((lang == 1) ? "Skills" : "Skills");
		case 931: return (lang == 2) ? "Efeitos Estaticos" : ((lang == 1) ? "Efectos Estaticos" : "Static Effects");
		case 932: return (lang == 2) ? "Efeitos Dinamicos" : ((lang == 1) ? "Efectos Dinamicos" : "Dynamic Effects");
		case 933: return (lang == 2) ? "Asas/Capas" : ((lang == 1) ? "Alas/Capas" : "Wings/Capes");
		case 934: return (lang == 2) ? "HP do Monstro" : ((lang == 1) ? "HP de Monstruo" : "Monster HP");
		case 935: return (lang == 2) ? "Interface" : ((lang == 1) ? "Interfaz" : "Interface");
		case 936: return (lang == 2) ? "PVP sem Ctrl" : ((lang == 1) ? "PVP sin Ctrl" : "PVP without Ctrl");
		case 937: return (lang == 2) ? "Renderizar Bordas" : ((lang == 1) ? "Renderizar Bordes" : "Render Borderless");
	}
	return "";
}
...
__declspec(naked) void COptionsMenu::OptionsWindowUI()
{
...
void COptionsMenu::RenderAllOptions()
{
	...
	sprintf_s(Text, "%s", this->GetOptionText(385));
	...
	sprintf_s(Text, "%s", this->GetOptionText(919));
	EnableAlphaTest(true);
	RenderText((int)PosX, CenterTextPosY(Text, (int)(PosY + (Height / 2.0f))), Text, REAL_WIDTH((int)Width), RT3_SORT_CENTER, NULL);
	PosY += (Height + 7.0f);
	...
	/* ANTILAG */
	this->RenderBox(PosX, PosY, Width, Height);
	sprintf_s(Text, "%s", this->GetOptionText(926));
	EnableAlphaTest(true);
	RenderText((int)PosX, CenterTextPosY(Text, (int)(PosY + (Height / 2.0f))), Text, REAL_WIDTH((int)Width), RT3_SORT_CENTER, NULL);
	PosY += (Height + 7.0f);
	...
	/* SCREEN */
	this->RenderBox(PosX, PosY, Width, Height);
	sprintf_s(Text, "%s", this->GetOptionText(920));
	EnableAlphaTest(true);
	RenderText((int)PosX, CenterTextPosY(Text, (int)(PosY + (Height / 2.0f))), Text, REAL_WIDTH((int)Width), RT3_SORT_CENTER, NULL);
	PosY += (Height + 7.0f);
	...
	/* FONT */
	this->RenderBox(PosX, PosY, Width, Height);
	sprintf_s(Text, "%s", this->GetOptionText(921));
	EnableAlphaTest(true);
	RenderText((int)PosX, CenterTextPosY(Text, (int)(PosY + (Height / 2.0f))), Text, REAL_WIDTH((int)Width), RT3_SORT_CENTER, NULL);
	PosY += (Height + 7.0f);
	...
	/* CLOSE */
	this->RenderBox(PosX, PosY, Width, Height);
	sprintf_s(Text, "%s", this->GetOptionText(388));
	EnableAlphaTest(true);
	RenderText((int)PosX, CenterTextPosY(Text, (int)(PosY + (Height / 2.0f))), Text, REAL_WIDTH((int)Width), RT3_SORT_CENTER, NULL);
	PosY += (Height + 7.0f);
}
...
bool COptionsMenu::CheckAllOptions()
{
	...
	{
		if (IsWorkZone((float)PosX, (float)PosY, (float)Width, (float)Height))
		{
...
void COptionsMenu::RenderLanguage(float PosX, float PosY)
{
	...
	{
		if (IsWorkZone((float)PosX, (float)PosY, (float)this->BoxHeight, (float)this->BoxHeight))
		{
			if (MouseLButtonPush)
			{
	...
	{
		if (IsWorkZone((float)PosX + (this->BoxWidth - this->BoxHeight), (float)PosY, (float)this->BoxHeight, (float)this->BoxHeight))
		{
			if (MouseLButtonPush)
			{
...
void COptionsMenu::RenderPVPWithoutControl(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s: %s", this->GetOptionText(936), this->PVPWithoutControl ? "On" : "Off");
...
{
	if (IsWorkZone((float)PosX, (float)PosY, (float)this->BoxWidth, (float)this->BoxHeight))
	{
		if (MouseLButton && MouseLButtonPush)
		{
			MouseLButtonPush = false;
...
void COptionsMenu::RenderAutomaticAttack(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s: %s", this->GetOptionText(386), m_bAutoAttack ? "On" : "Off");
...
void COptionsMenu::RenderSoundOnWhisper(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s: %s", this->GetOptionText(387), m_bWhisperSound ? "On" : "Off");
...
void COptionsMenu::RenderSoundVolume(float PosX, float PosY)
{
	...
	if (!m_SoundOnOff)
	{
		...
		sprintf_s(Text, "%s: Off", this->GetOptionText(922));
	...
	else
	{
		...
		sprintf_s(Text, "%s:", this->GetOptionText(922));
...
bool COptionsMenu::CheckSoundVolume(int PosX, int PosY)
{
	...
	if (MouseLButton && MouseLButtonPush)
	{
		...
		if (!m_SoundOnOff)
		{
			gSound.SetSoundEnabled(true);
			return true;
		}

		int LabelWidth = this->BoxWidth / 3;

		if (MouseX < PosX + LabelWidth)
		{
			gSound.SetSoundEnabled(false);
			return true;
		}

		float BarStartX = (float)(PosX + LabelWidth);

		float BarWidth = ((float)this->BoxWidth / 3.0f * 2.0f) - 8.0f;

		float ClickOffset = (float)MouseX - BarStartX;

		int newLevel = (int)((ClickOffset / BarWidth) * MAX_SOUND_LEVEL) + 1;

		if (newLevel < 1) newLevel = 1;
		if (newLevel > MAX_SOUND_LEVEL) newLevel = MAX_SOUND_LEVEL;

		gSound.UpdateSoundVolumeLevel(newLevel);
...
void COptionsMenu::RenderMusicVolume(float PosX, float PosY)
{
	...
	if (!m_MusicOnOff)
	{
		...
		sprintf_s(Text, "%s: Off", this->GetOptionText(923));
	...
	else
	{
		...
		sprintf_s(Text, "%s:", this->GetOptionText(923));
...
bool COptionsMenu::CheckMusicVolume(int PosX, int PosY)
{
	...
	if (MouseLButton && MouseLButtonPush)
	{
		...
		int newLevel = (int)((ClickOffset / BarWidth) * MAX_MUSIC_LEVEL) + 1;

		if (newLevel < 1) newLevel = 1;
		if (newLevel > MAX_MUSIC_LEVEL) newLevel = MAX_MUSIC_LEVEL;

		gSound.UpdateMusicVolumeLevel(newLevel);
...
bool COptionsMenu::CheckMusicControls(int PosX, int PosY)
{
	...
	if (IsWorkZone((float)StartPosX, (float)PosY, (float)Width, (float)this->BoxHeight))
	{
		if (MouseLButton && MouseLButtonPush)
		{
			MouseLButtonPush = false;
...
void COptionsMenu::RenderDeleteShadows(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s: %s", this->GetOptionText(927), this->DeleteShadows ? "On" : "Off");
...
void COptionsMenu::RenderDeleteObjects(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s: %s", this->GetOptionText(928), this->DeleteObjects ? "On" : "Off");
...
void COptionsMenu::RenderDeleteFloor(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s: %s", this->GetOptionText(929), this->DeleteFloor ? "On" : "Off");
...
void COptionsMenu::RenderDeleteSkills(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s: %s", this->GetOptionText(930), this->DeleteSkills ? "On" : "Off");
...
void COptionsMenu::RenderDeleteStaticEffects(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s: %s", this->GetOptionText(931), this->DeleteStaticEffects ? "On" : "Off");
...
void COptionsMenu::RenderDeleteDynamicEffects(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s: %s", this->GetOptionText(932), this->DeleteDynamicEffects ? "On" : "Off");
...
void COptionsMenu::RenderDeleteWings(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s: %s", this->GetOptionText(933), gWeaponView.DeleteWings ? "On" : "Off");
...
bool COptionsMenu::CheckDeleteWings(int PosX, int PosY)
{
	if (IsWorkZone((float)PosX, (float)PosY, (float)this->BoxWidth, (float)this->BoxHeight))
	{
		if (MouseLButton && MouseLButtonPush)
		{
			...
			char Text[10] = { 0 };
			wsprintf(Text, "%d", gWeaponView.DeleteWings);
...
void COptionsMenu::RenderDeleteHPBar(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s: %s", this->GetOptionText(934), MonsterHPBarMode ? "On" : "Off");
...
void COptionsMenu::RenderDeleteInterface(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s: %s", this->GetOptionText(935), this->DeleteInterface ? "On" : "Off");
...
void COptionsMenu::RenderResolutions(float PosX, float& PosY)
{
	...
	for (int i = 0; i < MAX_RESOLUTION_VALUE; i++)
	{
		...
		else if (IsWorkZone((float)PosX, (float)PosY, (float)this->BoxWidth, (float)this->BoxHeight))
		{
			EnableAlphaBlend();
...
void COptionsMenu::RenderResetFonts(float PosX, float& PosY)
{
	...
	sprintf_s(Text, "%s", this->GetOptionText(924));
...
bool COptionsMenu::CheckResetFonts(int PosX, int& PosY)
{
	if (IsWorkZone((float)PosX, (float)PosY, (float)this->BoxWidth, (float)this->BoxHeight))
	{
		if (MouseLButton && MouseLButtonPush)
		{
			...
			if (!gFont.AllFonts.empty())
			{
				sprintf_s(gFont.MyFontFaceName, "%s", gFont.AllFonts[0].c_str());
			}
...
void COptionsMenu::RenderBack(float PosX, float PosY)
{
	...
	sprintf_s(Text, "%s", this->GetOptionText(925));
