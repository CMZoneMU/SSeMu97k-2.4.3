...
void PartObjectColorHookC(int Type, float Alpha, float Bright, float Light[3], bool ExtraMon);
...
void CLoadModels::Init()
{
	...
	SetCompleteHook(0xE8, 0x0045A038, &PartObjectColorHookC);

	SetCompleteHook(0xE8, 0x00504A02, &PartObjectColorHookC);
}
...
void CLoadModels::CheckModelExists(int Model, char* SubFolder, char* filename)
{
	...
	if (fp == NULL)
	{
		LogAdd("[Model] '%s' file does not exist. Model number: %d", path, Model);
	}
	else
	{
		fclose(fp);
...
void CLoadModels::CheckTextureExists(int Model, char* SubFolder, char* filename, char* ext)
{
	...
	if (fp == NULL)
	{
		LogAdd("[Texture] '%s' file does not exist. Model number: %d", path, Model);
	}
	else
	{
		fclose(fp);
