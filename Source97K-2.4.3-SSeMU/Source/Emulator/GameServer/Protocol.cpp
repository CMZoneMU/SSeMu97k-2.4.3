...
void CGChatRecv(PMSG_CHAT_RECV* lpMsg,int aIndex) // OK
{
	...
	else if(lpMsg->message[0] == '@')
	{
		if(lpObj->Guild != 0)
		{
			if(lpMsg->message[1] == '>')
			{
				if(strcmp(lpObj->Name,lpObj->Guild->Master) == 0)
				{
					gGuild.GDGuildNoticeSend(lpObj->Guild->Name,&lpMsg->message[2]);
				}
			}
			else
			{
				gGuild.GDGuildPostSend(lpObj->Guild->Name,lpObj->Name,lpMsg->message);
			}
		}
	}
	else
...
void GCPartyItemInfoSend(int aIndex,CItem* lpItem) // OK
{
	...
	pMsg.ItemInfo = lpItem->m_Index & 0x1FF;
