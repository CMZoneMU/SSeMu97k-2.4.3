	...
	try
	{
		while (true)
		{
			...
			while (true)
			{
				...
				{
					info.SetAttr = lpMemScript->GetAsNumber();
				}
...
bool CItemManager::CheckItemIndex(int index) // OK
{
	ITEM_INFO ItemInfo;
...
int CItemManager::GetInventoryEmptySlotCount(LPOBJ lpObj) // OK
{
	...
	for (int n = 0; n < (INVENTORY_MAIN_SIZE - INVENTORY_WEAR_SIZE); n++)
	{
...
bool CItemManager::CheckItemInChaosBox(LPOBJ lpObj) // OK
{
...
bool CItemManager::CheckItemInTrade(LPOBJ lpObj) // OK
{
...
{
	for (int n = 0; n < INVENTORY_MAIN_SIZE, count > 0; n++)
	{
		if (lpObj->Inventory[n].IsItem() != 0)
		{
...
void CItemManager::DecreaseItemDur(LPOBJ lpObj, int slot, int dur) // OK
{
	...
	if (lpObj->Inventory[slot].m_Index == GET_ITEM(13, 18) || lpObj->Inventory[slot].m_Index == GET_ITEM(13, 29) || lpObj->Inventory[slot].m_Index == GET_ITEM(14, 19)) // Invisibility Cloak, Armor of Guardsman, Devil's Invitation
	{
...
void CItemManager::CGItemUseRecv(PMSG_ITEM_USE_RECV* lpMsg, int aIndex) // OK
{
	...
	{
		if (gFruit.UseFruitItem(lpObj, lpItem->m_Level) != 0)
		{
			this->InventoryDelItem(aIndex, lpMsg->SourceSlot);
			this->GCItemDeleteSend(aIndex, lpMsg->SourceSlot, 1);
		}
