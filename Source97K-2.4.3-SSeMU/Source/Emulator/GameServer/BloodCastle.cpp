		...
		while(true)
		{
			...
			while(true)
			{
				...
				else if(section == 5)
				{
					...
					this->m_BloodCastleSaintStatue[level][0] = lpMemScript->GetAsNumber();

					this->m_BloodCastleSaintStatue[level][1] = lpMemScript->GetAsNumber();
				}
				else
				{
					break;
...
void CBloodCastle::NpcAngelMessenger(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if(gItemManager.GetInventoryItemCount(lpObj,GET_ITEM(13,18),(level+1)) == 0 && gServerInfo.m_BloodCastleNPCWithoutEntrance == 0)
	{
