// Update 93 2.5.1 -> 97K - Sistema de resgate de presentes /gift
...
CCustomGift::CCustomGift()
{
	this->m_CustomGiftSwitch = 0;
...
void CCustomGift::ReadCustomGiftInfo(char* section,char* path)
{
	this->m_CustomGiftSwitch = GetPrivateProfileInt(section,"CustomGiftSwitch",0,path);
}

void CCustomGift::Load(char* path)
{
		...
		while (true)
		{
			...
			while (true)
			{
				if(section == 0)
				{
					...
					info.Money = lpMemScript->GetAsNumber(); // Coin1 (fallback)
					
					info.Money = lpMemScript->GetAsNumber(); // Coin2 (fallback)
...
void CCustomGift::SetInfo(GIFT_LIST_INFO info)
{
...
void CCustomGift::SetItemInfo(GIFT_LIST_ITEM_INFO info)
{
	std::map<int,GIFT_LIST_INFO>::iterator it = this->m_GiftListInfo.find(info.Index);
...
bool CCustomGift::CommandGift(LPOBJ lpObj,char* arg)
{
...
void CCustomGift::DGCommandGiftRecv(SDHP_COMMAND_GIFT_RECV* lpMsg)
{
